...
def CreateTorus(nameString, pos, rot, smooth):
    '''
        ...
        '''
    bpy.ops.mesh.primitive_torus_add(align='WORLD', location=pos, rotation=rot, major_segments=50, minor_segments=20, mode='MAJOR_MINOR', major_radius=0.25, minor_radius=0.11, abso_major_rad=0.5, abso_minor_rad=0.15, generate_uvs=True)
    obj = bpy.context.object
    if smooth == True:
        # Smooth the mesh
...
def CreatePlane(plane_loc,nameString):
    bpy.ops.mesh.primitive_plane_add(size=10, enter_editmode=False, location=plane_loc)
    obj = bpy.context.object
    bpy.context.active_object.name = nameString
...
def IcingDrops(dropsPercentage):
    '''
        ...
        '''
    bpy.ops.mesh.select_random(percent=dropsPercentage, seed=0, action='SELECT')
    bpy.ops.transform.translate(value=(-0.004, -0.0071, -0.034), orient_type='GLOBAL', orient_matrix=((1, 0, 0), (0, 1, 0), (0, 0, 1)), orient_matrix_type='GLOBAL', mirror=True, use_proportional_edit=True, proportional_edit_falloff='SMOOTH', proportional_size=0.118522, use_proportional_connected=False, use_proportional_projected=False)
