...
cora = bpy.data.texts["Cora.py"].as_module() # Script cointaining all the Utils
...
def Mode(modeString):
    ...
    if modeString.capitalize()=='Edit':
        ...
        raise NotImplementedError('Other types not implemented yet besides Edit and Obj')
...
print(scene.objects) # there should be 5 objects : light, camera, doughnut,icing, plane
...
bpy.ops.mesh.bisect(plane_co=(0, 0, 0.05), plane_no=(0, 0, 1), use_fill=False, clear_inner=True)
...
ice_mat = bpy.data.materials.new(name="icing_mat") #set new material to variable
activeObject.data.materials.append(ice_mat) #add the material to the object
ice_mat.use_nodes = True
#bpy.context.object.active_material_index = 0
bpy.data.materials["icing_mat"].node_tree.nodes["Principled BSDF"].inputs['Base Color'].default_value = (0.8, 0.23, 0.56, 1)
bpy.data.materials["icing_mat"].node_tree.nodes["Principled BSDF"].inputs['Subsurface Color'].default_value = (0.8, 0.23, 0.44, 1)
...
bpy.data.materials["icing_mat"].node_tree.nodes["Principled BSDF"].inputs[17].default_value = (0.39, 0.006, 0.26, 1)
...
dou_mat = bpy.data.materials.new(name="dou_mat") #set new material to variable
...
bpy.data.materials["dou_mat"].node_tree.nodes["Principled BSDF"].inputs[0].default_value = (0.8, 0.58, 0.34, 1)
bpy.data.materials["dou_mat"].node_tree.nodes["Principled BSDF"].inputs[3].default_value = (0.8, 0.47, 0.4, 1)
...
plane_mat = bpy.data.materials.new(name="plane_mat") #set new material to variable
activeObject3.data.materials.append(plane_mat) #add the material to the object
...
bpy.data.materials["plane_mat"].node_tree.nodes["Principled BSDF"].inputs[0].default_value = (0.8, 0.3, 0.7, 1)
bpy.data.materials["plane_mat"].node_tree.nodes["Principled BSDF"].inputs[1].default_value = 0.22
bpy.data.materials["plane_mat"].node_tree.nodes["Principled BSDF"].inputs[3].default_value = (0.8, 0.38, 0.74, 1)
...
bpy.data.materials["dou_mat"].node_tree.nodes["Mix"].inputs[2].default_value = (0.5, 0.2, 0.045, 1)
...
link2 = links.new(overlay.outputs['Color'],principled_dou.inputs['Base Color'])
...
bpy.data.materials["plane_mat"].node_tree.nodes["RGB"].outputs[0].default_value = (0.4, 0.04, 1, 1)
...
link14 = links2.new(principled_plane.inputs['Base Color'],mix.outputs['Color'])
...
link21 = links2.new(planeOut.inputs['Displacement'],displacement.outputs['Displacement'])
...
if nodeDenoise is not None:
    ...
    Link01 = compositorLinks.new(denoise.inputs['Image'],renderL.outputs["Noisy Image"])
    Link02 = compositorLinks.new(denoise.inputs['Normal'],renderL.outputs["Denoising Normal"])
    Link03 = compositorLinks.new(denoise.inputs['Albedo'],renderL.outputs["Denoising Albedo"])
    Link04 = compositorLinks.new(composite.inputs['Image'],denoise.outputs['Image'])
